...
def main():
    ...
    Filter.load_processed_corpus(path="data/processed/")
...
class ArticleFilter(object):
    ...
    def load_processed_corpus(self, path="data/processed/"):
        ...
        # print('test: ', self.corpus[1]["Title"])
        # print('test: ', self.corpus[1]["Responses"])  # 有些沒標題
        ...
        for article in self.corpus:
            ...
            self.titles.add(temp_title)
            self.order_titles.append(temp_title)

            try:
                self.order_response.append(article["Responses"])
            except:
                self.order_response.append([])
